...
export default function Navbar({ className }) {
  ...
  const links = liObj.map(({ to, label }) => {
    ...
    return (
      <NavLink className={({isActive})=> isActive ? 'font-bold text-rose-500 p-2 shadow-md/30 border-b border-b-3  border-red-500 bg-indigo-100 rounded-md px-5' : liStyle} key={to} to={to}>
        {label}
      </NavLink>
    );
  });
  ...
  return (
    <nav className={`${className}  bg-indigo-500 p-1  shadow-md/30 text-white`}>
