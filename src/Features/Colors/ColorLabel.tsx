...
export function ColorLabel({
  ...
  const colorLabelOrForm = {
    colorLabel: (
      <div
        className={`m-2 flex flex-row gap-7 items-center ${border}  p-3  font-bold`}
      >
  ...
  const handleDeleteClick = () => {
    deleteColor(`${ApiLinks.color.delete}`, {
      data: { ColorId: color.colorId },
    });
  };
  ...
  return (
    <CardLabel
      ...
      handleUpdateMode={handleClickUpdateMode}
    >
      {content}
    </CardLabel>
  );
  return (
    <div className="m-2 flex flex-col  justify-between shadow-md/10  rounded-xl p-3 bg-gray-100 font-bold text-indigo-800">
      {content}
      <div className="m-2 flex flex-row  justify-between text-xl     shadow p-1 rounded-xl bg-white font-bold">
        <MdDeleteForever
          onClick={handleDeleteClick}
          className="cursor-pointer bg-red-500  rounded-sm  text-white"
        />
        <MdOutlineCreate
          onClick={handleClickUpdateMode}
          className="cursor-pointer bg-blue-500  rounded-sm  text-white"
        />
      </div>
    </div>
  );
