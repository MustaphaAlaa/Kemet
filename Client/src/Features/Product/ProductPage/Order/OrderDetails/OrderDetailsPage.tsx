...
export default function OrderDetailsPage() {
    ...
    const [governorateDeliveryCompanyCost, setGovernorateDeliveryCompanyCost] = useState(info.governorateDeliveryCompanyCost);

    const selectDeliveryCompany = <SelectDeliveryCompany
        deliveryCompanyId={info.deliveryCompanyId}
        setGovernorateDeliveryCompanyCost={setGovernorateDeliveryCompanyCost}
        orderId={orderId}
        governorateId={info.governorateId}></SelectDeliveryCompany>;
    ...
    return (
        <div className='p-5 flex flex-col justify-between   space-y-8 '>
            ...
            <div className='w-2/3 mx-auto bg-indigo-100  rounded-xl overflow-hidden shadow-xl/40 shadow-indigo-800 border-2 border-indigo-400'>
                ...
                {info.orderStatus == 1 || info.orderStatus == 2 ? selectDeliveryCompany : <ShowDeliveryCompany deliveryCompanyId={info.deliveryCompanyId}></ShowDeliveryCompany>}
            </div>
            {/* This card should be have accounting for this order 
                eg.. 
                total, returns, deliver cost, etc.. 
            */}
            <div className='md:w-[80%] xl:w-2/3  p-3  md:mx-auto flex flex-col items-center  bg-gradient-to-l from-white to-gray-100 border-3 border-cyan-900 rounded-xl'>
                <div className='md:w-2/3 w-[80%] p-3  flex flex-row justify-between items-center border-b-1 border-b-gray-300'>
                ...
                <div className='md:w-2/3 w-[80%] p-3  flex flex-row justify-between items-center border-b-1 border-b-gray-300'>
                    ...
                    <div className='text-[1.2rem]  font-semibold flex flex-row justify-between items-center space-x-2'>
                        <p>
                            ...
                            {governorateDeliveryCompanyCost ?? '-'}
                        </p>
                        <p>ج.م</p>
                    </div>
                </div>
                <div className='md:w-2/3 w-[80%] p-3  flex flex-row justify-between items-center '>
                    <div className='text-[1.5rem] text-blue-700 font-bold'> سعر توصيل  للعميل</div>
                    <div className='text-[1.2rem]  font-semibold flex flex-row justify-between items-center space-x-2'>
                        <p>
